...
{\LARGE \textbf{Asymmetric Cost-Sensitive Ensemble Credit Risk Assessment}}\\[0.8cm]
{\large Using Ant Colony Optimisation and Quadratic Programming}\\[2.5cm]

{\large \textbf{Project Team}}\\[0.6cm]
{\normalsize
[Team Member 1 --- Roll No.]\\[0.3cm]
[Team Member 2 --- Roll No.]\\[0.3cm]
[Team Member 3 --- Roll No.]\\[0.3cm]
[Team Member 4 --- Roll No.]\\[0.3cm]
[Team Member 5 --- Roll No.]\\[1.5cm]
}

{\normalsize Submitted in partial fulfilment of the requirements for}\\[0.3cm]
{\normalsize [Course Name and Code]}\\[0.3cm]
{\normalsize [Department of Computer Science / Information Technology]}\\[0.3cm]
{\normalsize [Institution Name]}\\[1.5cm]

{\normalsize Supervised by: [Supervisor Name]}\\[2cm]
...
Instalment rate (\%) & Instalment as \% of disposable income & 1 -- 4 & Integer-valued but treated as ordinal continuous \\
...
Number of dependents & Number of dependants to provide for & 1 -- 2 & Binary integer; 84\% of applicants have 1 dependent \\
\bottomrule
\end{tabular}
\end{table}
...
Other instalment plans & Instalment plans at other institutions & Bank; stores; none \\
...
Numerical features are scaled using \textbf{RobustScaler}, which centres each feature by its median and scales by its interquartile range (IQR):
\[
...
\begin{enumerate}
    \item For each minority sample, identifying its $k=5$ nearest neighbours in the RobustScaled continuous feature space.
    ...
    \item For categorical features, selecting the mode of the $k$ nearest neighbours, preserving valid category membership.
\end{enumerate}
...
found by direct linear cost minimisation on the isolated $P_\text{calib}$ partition
...
The nested cross-validation with HPO requires approximately 828 pipeline fits across three models. Runtime on standard cloud hardware is approximately 15--45 minutes.
...
The weight optimisers (QP and ACO) minimise MSE, while the deployment objective is asymmetric cost minimisation. Replacing the MSE objective with a direct cost function would render QP inapplicable, making ACO the only valid solver.

\subsection{MLP Inductive Bias}

The ReLU MLP is structurally mismatched to this dataset: after one-hot encoding, the majority of features are binary, and ReLU provides no non-linear transformation on binary inputs.
...
\begin{itemize}
    \item \textbf{Fuzziness (Vagueness)}: The inherent gradedness of linguistic categories. The boundary between ``Low Risk'' and ``Medium Risk'' is not crisp; a probability of 0.25 is ``more Low Risk'' and ``less Medium Risk'' by degree. Fuzzy membership functions, as used in ANFIS, model this gradedness.
